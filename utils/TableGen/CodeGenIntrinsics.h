...
struct CodeGenIntrinsic {
  ...
  struct IntrinsicSignature {
    ...
    IntrinsicSignature() = default;
    IntrinsicSignature(const IntrinsicSignature& RHS) = default;
    IntrinsicSignature(IntrinsicSignature&& RHS) = default;

    IntrinsicSignature& operator=(const IntrinsicSignature& RHS) {
      if (this != &RHS) {
        RetVTs = RHS.RetVTs;
        RetTypeDefs = RHS.RetTypeDefs;
        ParamVTs = RHS.ParamVTs;
        ParamTypeDefs = RHS.ParamTypeDefs;
      }

      return *this;
    }

    const IntrinsicSignature& operator=(IntrinsicSignature&& RHS) {
      if (this != &RHS) {
        RetVTs = std::move(RHS.RetVTs);
        RetTypeDefs = std::move(RHS.RetTypeDefs);
        ParamVTs = std::move(RHS.ParamVTs);
        ParamTypeDefs = std::move(RHS.ParamTypeDefs);
      }
  ...
  CodeGenIntrinsic() = default;
  CodeGenIntrinsic(const CodeGenIntrinsic& RHS) = default;
  CodeGenIntrinsic(CodeGenIntrinsic&& RHS) = default;

  CodeGenIntrinsic& operator=(const CodeGenIntrinsic& RHS) {
    if (this != &RHS) {
      TheDef = RHS.TheDef;
      Name = RHS.Name;
      EnumName = RHS.EnumName;
      GCCBuiltinName = RHS.GCCBuiltinName;
      MSBuiltinName = RHS.MSBuiltinName;
      TargetPrefix = RHS.TargetPrefix;
      IS = RHS.IS;
      ModRef = RHS.ModRef;
      Properties = RHS.Properties;
      isOverloaded = RHS.isOverloaded;
      isCommutative = RHS.isCommutative;
      canThrow = RHS.canThrow;
      isNoDuplicate = RHS.isNoDuplicate;
      isNoReturn = RHS.isNoReturn;
      isConvergent = RHS.isConvergent;
      hasSideEffects = RHS.hasSideEffects;
      isSpeculatable = RHS.isSpeculatable;
      ArgumentAttributes = RHS.ArgumentAttributes;
    }

    return *this;
  }

  CodeGenIntrinsic& operator=(CodeGenIntrinsic&& RHS) {
    if (this != &RHS) {
      TheDef = std::move(RHS.TheDef);
      Name = std::move(RHS.Name);
      EnumName = std::move(RHS.EnumName);
      GCCBuiltinName = std::move(RHS.GCCBuiltinName);
      MSBuiltinName = std::move(RHS.MSBuiltinName);
      TargetPrefix = std::move(RHS.TargetPrefix);
      IS = std::move(RHS.IS);
      ModRef = std::move(RHS.ModRef);
      Properties = std::move(RHS.Properties);
      isOverloaded = std::move(RHS.isOverloaded);
      isCommutative = std::move(RHS.isCommutative);
      canThrow = std::move(RHS.canThrow);
      isNoDuplicate = std::move(RHS.isNoDuplicate);
      isNoReturn = std::move(RHS.isNoReturn);
      isConvergent = std::move(RHS.isConvergent);
      hasSideEffects = std::move(RHS.hasSideEffects);
      isSpeculatable = std::move(RHS.isSpeculatable);
      ArgumentAttributes = std::move(RHS.ArgumentAttributes);
    }
